...
| **Remote Desktop Control** | `src/routes/remote-desktop.tsx`<br>`src/features/remote-desktop/RemoteDesktopPage.tsx`<br>`src/features/remote-desktop/stores/rd-host.store.ts`<br>`src/features/remote-desktop/stores/rd-controller.store.ts`<br>`native-agent/server.mjs` | `useHostRemoteDesktop`<br>`useControllerRemoteDesktop` |
